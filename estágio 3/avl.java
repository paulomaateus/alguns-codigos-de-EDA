...
class arvore{
    ...
    public void balancear(Node node){
        Node pai;
        Node avo;
        if(node.getAnterior() != null && node.getAnterior().getAnterior() != null){
                pai = node.getAnterior();
                avo = pai.getAnterior();
            if(!avo.balanceado()){
                if(avo.pendeDireita() && pai.filhoDireita(node)){
                    rotacionaEsq(avo);
                }else if(avo.pendeEsquerda() && pai.filhoEsquerda(node)){
                    rotacionaDir(avo);
                }else if(avo.pendeDireita() && pai.filhoEsquerda(node)){
                    rotacionaDir(pai);
                    rotacionaEsq(avo);
                }else{
                    rotacionaEsq(pai);
